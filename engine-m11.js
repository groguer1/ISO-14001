/* Extensión del motor para el Módulo 11 — Proyecto Final (wizard de slides): Implementa tu SGA. */
(function () {
  var Deck = window.Deck;
  var PASS_SCORE = 8; // sobre 10

  var EMPRESAS = [
    { id: 'textil', icon: '🧵', nombre: 'Fábrica textil', desc: '35 empleados, teñido y confección' },
    { id: 'metal', icon: '⚙️', nombre: 'Taller de mecanizado', desc: '20 empleados, residuos de aceites y metales' },
    { id: 'alimentaria', icon: '🥫', nombre: 'Planta agroalimentaria', desc: '50 empleados, alto consumo de agua y energía' },
    { id: 'logistica', icon: '🚚', nombre: 'Empresa de logística', desc: '40 empleados, flota de vehículos y almacén' },
    { id: 'construccion', icon: '🏗️', nombre: 'Constructora', desc: '60 empleados, gestión de residuos de obra' },
    { id: 'propia', icon: '🏢', nombre: 'Mi organización real', desc: 'Usa tu propia organización' }
  ];

  var CAMPOS = [
    { id: 'p1_alcance', min: 20, nombre: 'Alcance del SGA' },
    { id: 'p1_politica', min: 25, nombre: 'Política ambiental' },
    { id: 'p2_aspectos', min: 25, nombre: 'Aspectos e impactos ambientales' },
    { id: 'p2_legal', min: 20, nombre: 'Requisitos legales aplicables' },
    { id: 'p3_objetivos', min: 30, nombre: 'Objetivos ambientales SMART' },
    { id: 'p3_control', min: 20, nombre: 'Control operacional' },
    { id: 'p4_emergencias', min: 20, nombre: 'Plan de emergencias ambientales' },
    { id: 'p5_kpis', min: 20, nombre: 'Indicadores de desempeño ambiental' },
    { id: 'p5_auditoria', min: 15, nombre: 'Plan de auditoría interna' },
    { id: 'p5_revision', min: 15, nombre: 'Revisión por la dirección' }
  ];
  ...
  Deck.render_evaluar = function () {
    ...
    wrap.innerHTML = '<h2 style="text-align:center;">Evalúa tu proyecto</h2>' +
      '<p style="text-align:center;">Cuando hayas completado las secciones, evalúa tu SGA. No hay respuestas incorrectas: se valora que cada sección esté suficientemente desarrollada y sea coherente con la organización elegida.</p>' +
      ...
      '<button class="btn-nav secundario" id="btnExportar">📄 Exportar mi SGA en PDF</button>' +
    ...
    wrap.querySelector('#btnExportar').addEventListener('click', function () {
      ...
      var lines = ['SISTEMA DE GESTIÓN AMBIENTAL — ISO 14001:2015', '='.repeat(50), ''];
      ...
      buildDocumentPDF('Proyecto final — Sistema de Gestión Ambiental', lines.join('\n'), 'sga-proyecto-final-iso14001.pdf');
    });
  ...
  Deck.render_certificado = function () {
    ...
    wrap.innerHTML =
      ...
      '<div id="certNombrePreview" style="font-family:\'DM Serif Display\',serif;font-size:28px;margin-bottom:18px;">' + (p.nombreAlumno ? esc_(p.nombreAlumno) : 'Profesional en Gestión Ambiental') + '</div>' +
      '<p style="font-size:14px;color:rgba(255,255,255,0.85);max-width:560px;margin:0 auto 18px;">ha completado satisfactoriamente el curso <strong style="color:#fff;">Gestor ISO 14001:2015 — Implementador de Sistemas de Gestión Ambiental</strong>, con proyecto final aprobado (' + p.score + '/10 puntos).</p>' +
      '<div style="font-size:12.5px;color:rgba(255,255,255,0.6);">11 módulos · 25 horas · 120 preguntas de evaluación · ' + fecha + '</div>' +
      '</div>' +
    ...
    wrap.querySelector('#inputNombre').addEventListener('input', function (e) {
      ...
      self.persist();
      wrap.querySelector('#certNombrePreview').textContent = p.nombreAlumno || 'Profesional en Gestión Ambiental';
    });
    wrap.querySelector('#btnDescargarCert').addEventListener('click', function () {
      buildCertificatePDF({ nombre: p.nombreAlumno || 'Profesional en Gestión Ambiental', puntuacion: p.score, fecha: fecha });
    });
    return wrap;
  };
  ...
  function buildCertificatePDF(data) {
    ...
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(12.5);
    var body = 'por haber completado satisfactoriamente el curso "Gestor ISO 14001:2015 — Implementador de Sistemas de Gestión Ambiental",\ndemostrando competencia en el diseño, implantación, control operacional y auditoría de Sistemas\nde Gestión Ambiental conforme a los requisitos de la norma ISO 14001:2015.';
    var by = 250;
    ...
    doc.setFont('helvetica', 'bold');
    doc.text('ISO 14001', cx, sigY - 12, { align: 'center' });
    doc.text('2015', cx, sigY - 2, { align: 'center' });

    doc.setFontSize(8.5);
    doc.setTextColor(150, 158, 170);
    doc.setFont('helvetica', 'normal');
    doc.text('ISO 14001:2015 · 25 horas · 120 preguntas de evaluación · Proyecto final', cx, h - 58, { align: 'center' });

    doc.save('certificado-iso14001-' + data.nombre.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') + '.pdf');
  }
})();
